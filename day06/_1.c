...
typedef struct NODE {
 ...
 if(newNode != NULL) {
  ...
  // 연결 리스트의 끝에 새로운 노드를 추가하는 함수 : 후위삽입 
...
node* searchNode(node *head, int data) {
    // 현재 노드를 가리키는 포인터 변수 선언
    node* current = head->next; // 더미 노드는 건너뜁니다.

    // 연결 리스트를 순회하면서 데이터를 탐색
    while (current != NULL) {
        // 현재 노드의 데이터와 찾고자 하는 데이터가 일치하는지 확인
        if (current->data == data) {
            // 일치하는 데이터를 찾았으므로 현재 노드의 포인터 반환
            return current;
        }
        // 다음 노드로 이동
        current = current->next;
